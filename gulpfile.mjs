...
function handleError(taskName, err, stream) {
  ...
  if (err.lineNumber || err.line)
    console.error("📏 Linha:", err.lineNumber || err.line);
  if (err.columnNumber || err.column)
    console.error("📐 Coluna:", err.columnNumber || err.column);
      ...
      .pipe(
        data((file) => {
          try {
            ...
            console.error(
              `❌ Erro ao carregar dados JSON para ${file.path}:`,
              err.message,
            );
            return {};
          }
...
export function css(done) {
  try {
    // Compilação individual de arquivos SCSS mantendo estrutura
    gulp
      .src("./src/blocks/*.scss", { base: "src" })
      .pipe(
        plumber({
          errorHandler: function (err) {
      ...
      .pipe(
        postcss([autoprefixer(), cssnano()]).on("error", function (err) {
...
export function cssUnified(done) {
  try {
    ...
    gulp
      ...
      .pipe(
        postcss([autoprefixer()]).on("error", function (err) {
...
function watchTask() {
  try {
    ...
    const sassWatcher = gulp.watch(
      "./src/**/*.scss",
      gulp.series(css, cssUnified),
    );
    sassWatcher.on("error", (err) => {
      console.error("❌ Erro no watcher SCSS:", err.message);
    });
    ...
    const jsWatcher = gulp.watch(
      "./src/**/*.js",
      gulp.series(javascript, javascriptUnified),
    );
